...
// Model for BakeryCountry entity
    models.BakeryCountry = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakerycountry/",
        idAttribute: 'bakeryCountryId',
        defaults: {
            countryName: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('bakeryCountryId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakerycountry/';
            }
    ...
    // Collection class for BakeryCountry entities
    models.BakeryCountryCollection = Backbone.Collection.extend({
        model: models.BakeryCountry,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakerycountry/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.BakeryCountryCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-bakerycountry-list-itemis template identifier for item
                        templateName: '#tpl-bakerycountry-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.bakerycountry = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.bakerycountry,
                    // tpl-bakerycountry-details is template identifier for chosen model element
                    templateName: '#tpl-bakerycountry-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.BakeryCountry();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-bakerycountry-details is a template identifier for chosen model element
                templateName: '#tpl-bakerycountry-details',
        ...
        getData: function () {
            return {
                bakeryCountryId: $('#bakeryCountryId').val(),
