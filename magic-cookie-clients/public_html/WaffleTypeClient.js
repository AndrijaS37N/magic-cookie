...
// Model for WaffleType entity
    models.WaffleType = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.waffletype/",
        idAttribute: 'waffleTypeId',
        defaults: {
            waffleTypeName: "",
            waffleTypeDescription: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('waffleTypeId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.waffletype/';
            }
    ...
    // Collection class for WaffleType entities
    models.WaffleTypeCollection = Backbone.Collection.extend({
        model: models.WaffleType,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.waffletype/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.WaffleTypeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-waffletype-list-itemis template identifier for item
                        templateName: '#tpl-waffletype-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.waffletype = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.waffletype,
                    // tpl-waffletype-details is template identifier for chosen model element
                    templateName: '#tpl-waffletype-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.WaffleType();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-waffletype-details is a template identifier for chosen model element
                templateName: '#tpl-waffletype-details',
        ...
        getData: function () {
            return {
                waffleTypeId: $('#waffleTypeId').val(),
                waffleTypeName: $('#waffleTypeName').val(),
                waffleTypeDescription: $('#waffleTypeDescription').val()
            };
        }
    });
    new AppRouter();
