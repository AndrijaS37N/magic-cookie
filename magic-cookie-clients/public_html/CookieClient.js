...
// Model for Cookie entity
    models.Cookie = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.cookie/",
        idAttribute: 'cookieId',
        defaults: {
            cookieDescription: "",
            cookieImage: "",
            cookieName: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('cookieId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.cookie/';
            }
    ...
    // Collection class for Cookie entities
    models.CookieCollection = Backbone.Collection.extend({
        model: models.Cookie,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.cookie/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CookieCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-cookie-list-itemis template identifier for item
                        templateName: '#tpl-cookie-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.cookie = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.cookie,
                    // tpl-cookie-details is template identifier for chosen model element
                    templateName: '#tpl-cookie-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Cookie();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-cookie-details is a template identifier for chosen model element
                templateName: '#tpl-cookie-details',
        ...
        getData: function () {
            return {
                ...
                cookieDescription: $('#cookieDescription').val(),
                cookieImage: $('#cookieImage').val(),
