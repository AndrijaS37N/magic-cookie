...
// Model for ChocolateType entity
    models.ChocolateType = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.chocolatetype/",
        idAttribute: 'chocolateTypeId',
        defaults: {
            chocolateTypeName: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('chocolateTypeId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.chocolatetype/';
            }
    ...
    // Collection class for ChocolateType entities
    models.ChocolateTypeCollection = Backbone.Collection.extend({
        model: models.ChocolateType,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.chocolatetype/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ChocolateTypeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-chocolatetype-list-itemis template identifier for item
                        templateName: '#tpl-chocolatetype-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.chocolatetype = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.chocolatetype,
                    // tpl-chocolatetype-details is template identifier for chosen model element
                    templateName: '#tpl-chocolatetype-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ChocolateType();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-chocolatetype-details is a template identifier for chosen model element
                templateName: '#tpl-chocolatetype-details',
        ...
        getData: function () {
            return {
                chocolateTypeId: $('#chocolateTypeId').val(),
                chocolateTypeName: $('#chocolateTypeName').val()
            };
        }
    });
    new AppRouter();
