...
// Model for Treat entity
    models.Treat = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.treat/",
        idAttribute: 'treatId',
        defaults: {
            treatDescription: "",
            howWasItMade: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('treatId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.treat/';
            }
    ...
    // Collection class for Treat entities
    models.TreatCollection = Backbone.Collection.extend({
        model: models.Treat,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.treat/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.TreatCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-treat-list-itemis template identifier for item
                        templateName: '#tpl-treat-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.treat = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.treat,
                    // tpl-treat-details is template identifier for chosen model element
                    templateName: '#tpl-treat-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Treat();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-treat-details is a template identifier for chosen model element
                templateName: '#tpl-treat-details',
        ...
        getData: function () {
            return {
                treatId: $('#treatId').val(),
                treatDescription: $('#treatDescription').val(),
                howWasItMade: $('#howWasItMade').val(),
                treatImage: $('#treatImage').val(),
