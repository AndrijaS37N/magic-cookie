...
// Model for UserAdmin entity
    models.UserAdmin = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.useradmin/",
        idAttribute: 'adminNumber',
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.useradmin/';
            }
    ...
    // Collection class for UserAdmin entities
    models.UserAdminCollection = Backbone.Collection.extend({
        model: models.UserAdmin,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.useradmin/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UserAdminCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-useradmin-list-itemis template identifier for item
                        templateName: '#tpl-useradmin-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.useradmin = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    ...
                    // tpl-useradmin-details is template identifier for chosen model element
                    templateName: '#tpl-useradmin-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.UserAdmin();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-useradmin-details is a template identifier for chosen model element
                templateName: '#tpl-useradmin-details',
