...
// Model for BakeryCity entity
    models.BakeryCity = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakerycity/",
        idAttribute: 'bakeryCityId',
        defaults: {
            cityName: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('bakeryCityId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakerycity/';
            }
    ...
    // Collection class for BakeryCity entities
    models.BakeryCityCollection = Backbone.Collection.extend({
        model: models.BakeryCity,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakerycity/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.BakeryCityCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-bakerycity-list-itemis template identifier for item
                        templateName: '#tpl-bakerycity-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.bakerycity = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.bakerycity,
                    // tpl-bakerycity-details is template identifier for chosen model element
                    templateName: '#tpl-bakerycity-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.BakeryCity();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-bakerycity-details is a template identifier for chosen model element
                templateName: '#tpl-bakerycity-details',
