...
// Model for CookieType entity
    models.CookieType = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.cookietype/",
        idAttribute: 'cookieTypeId',
        defaults: {
            cookieTypeName: "",
            numberOfChips: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('cookieTypeId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.cookietype/';
            }
    ...
    // Collection class for CookieType entities
    models.CookieTypeCollection = Backbone.Collection.extend({
        model: models.CookieType,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.cookietype/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CookieTypeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-cookietype-list-itemis template identifier for item
                        templateName: '#tpl-cookietype-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.cookietype = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.cookietype,
                    // tpl-cookietype-details is template identifier for chosen model element
                    templateName: '#tpl-cookietype-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.CookieType();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-cookietype-details is a template identifier for chosen model element
                templateName: '#tpl-cookietype-details',
        ...
        getData: function () {
            return {
                cookieTypeId: $('#cookieTypeId').val(),
                cookieTypeName: $('#cookieTypeName').val(),
                numberOfChips: $('#numberOfChips').val(),
                cookieTypeDescription: $('#cookieTypeDescription').val()
            };
        }
    });
    new AppRouter();
