...
// Model for UserEmployee entity
    models.UserEmployee = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.useremployee/",
        idAttribute: 'employeeNumber',
        defaults: {
            employeeDescription: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('employeeNumber');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.useremployee/';
            }
    ...
    // Collection class for UserEmployee entities
    models.UserEmployeeCollection = Backbone.Collection.extend({
        model: models.UserEmployee,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.useremployee/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UserEmployeeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-useremployee-list-itemis template identifier for item
                        templateName: '#tpl-useremployee-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.useremployee = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.useremployee,
                    // tpl-useremployee-details is template identifier for chosen model element
                    templateName: '#tpl-useremployee-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.UserEmployee();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-useremployee-details is a template identifier for chosen model element
                templateName: '#tpl-useremployee-details',
        ...
        getData: function () {
            return {
                ...
                employeeDescription: $('#employeeDescription').val()
            };
        }
    });
    new AppRouter();
