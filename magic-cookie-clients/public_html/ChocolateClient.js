...
// Model for Chocolate entity
    models.Chocolate = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.chocolate/",
        idAttribute: 'chocolateId',
        defaults: {
            costPerBlock: "",
            chocolateName: "",
            chocolateImage: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('chocolateId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.chocolate/';
            }
    ...
    // Collection class for Chocolate entities
    models.ChocolateCollection = Backbone.Collection.extend({
        model: models.Chocolate,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.chocolate/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ChocolateCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-chocolate-list-itemis template identifier for item
                        templateName: '#tpl-chocolate-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.chocolate = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.chocolate,
                    // tpl-chocolate-details is template identifier for chosen model element
                    templateName: '#tpl-chocolate-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Chocolate();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-chocolate-details is a template identifier for chosen model element
                templateName: '#tpl-chocolate-details',
        ...
        getData: function () {
            return {
                chocolateId: $('#chocolateId').val(),
                costPerBlock: $('#costPerBlock').val(),
                chocolateName: $('#chocolateName').val(),
                chocolateImage: $('#chocolateImage').val(),
                chocolateDescription: $('#chocolateDescription').val()
            };
        }
    });
    new AppRouter();
