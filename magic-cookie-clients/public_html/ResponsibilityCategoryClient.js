...
// Model for ResponsibilityCategory entity
    models.ResponsibilityCategory = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.responsibilitycategory/",
        idAttribute: 'responsibilityCategoryId',
        defaults: {
            responsibilityName: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('responsibilityCategoryId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.responsibilitycategory/';
            }
    ...
    // Collection class for ResponsibilityCategory entities
    models.ResponsibilityCategoryCollection = Backbone.Collection.extend({
        model: models.ResponsibilityCategory,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.responsibilitycategory/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ResponsibilityCategoryCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-responsibilitycategory-list-itemis template identifier for item
                        templateName: '#tpl-responsibilitycategory-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.responsibilitycategory = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.responsibilitycategory,
                    // tpl-responsibilitycategory-details is template identifier for chosen model element
                    templateName: '#tpl-responsibilitycategory-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ResponsibilityCategory();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-responsibilitycategory-details is a template identifier for chosen model element
                templateName: '#tpl-responsibilitycategory-details',
        ...
        getData: function () {
            return {
                responsibilityCategoryId: $('#responsibilityCategoryId').val(),
