...
// Model for Waffle entity
    models.Waffle = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.waffle/",
        idAttribute: 'waffleId',
        defaults: {
            costPerPortion: "",
            waffleStuffing: "",
            waffleImage: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('waffleId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.waffle/';
            }
    ...
    // Collection class for Waffle entities
    models.WaffleCollection = Backbone.Collection.extend({
        model: models.Waffle,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.waffle/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.WaffleCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-waffle-list-itemis template identifier for item
                        templateName: '#tpl-waffle-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.waffle = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.waffle,
                    // tpl-waffle-details is template identifier for chosen model element
                    templateName: '#tpl-waffle-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Waffle();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-waffle-details is a template identifier for chosen model element
                templateName: '#tpl-waffle-details',
        ...
        getData: function () {
            return {
                waffleId: $('#waffleId').val(),
                costPerPortion: $('#costPerPortion').val(),
                waffleStuffing: $('#waffleStuffing').val(),
                waffleImage: $('#waffleImage').val(),
                waffleName: $('#waffleName').val(),
                waffleDescription: $('#waffleDescription').val()
            };
        }
    });
    new AppRouter();
