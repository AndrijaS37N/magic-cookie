...
// Model for Bakery entity
    models.Bakery = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakery/",
        idAttribute: 'bakeryId',
        defaults: {
            startsWorking: "",
            squareMeters: "",
            tables: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('bakeryId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakery/';
            }
    ...
    // Collection class for Bakery entities
    models.BakeryCollection = Backbone.Collection.extend({
        model: models.Bakery,
        url: "http://localhost:8080/magic-cookie-services/webresources/my.company.mc.pr.bakery/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.BakeryCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-bakery-list-itemis template identifier for item
                        templateName: '#tpl-bakery-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.bakery = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.bakery,
                    // tpl-bakery-details is template identifier for chosen model element
                    templateName: '#tpl-bakery-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Bakery();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-bakery-details is a template identifier for chosen model element
                templateName: '#tpl-bakery-details',
        ...
        getData: function () {
            return {
                bakeryId: $('#bakeryId').val(),
                startsWorking: $('#startsWorking').val(),
                squareMeters: $('#squareMeters').val(),
                tables: $('#tables').val(),
                bakeryAddress: $('#bakeryAddress').val(),
                bakeryPhone: $('#bakeryPhone').val(),
                endsWorking: $('#endsWorking').val(),
                bakeryTwitter: $('#bakeryTwitter').val(),
                bakeryDescription: $('#bakeryDescription').val()
            };
        }
    });
    new AppRouter();
